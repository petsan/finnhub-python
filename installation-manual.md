...
| `FINN_PREDICTOR_SCORER` | `vader` | Which sentiment scorer the live pipeline uses. `vader` (default) needs no extra dependencies. `finbert` switches to FinBERT — requires `pip install torch transformers` and downloads ~440 MB of weights on first use. The training loop reads from the matching `model_version` automatically, so set this once and retrain. |
| `FINN_PREDICTOR_CLASSIFIER` | `rule` | How the final UP/DOWN/FLAT decision is taken. `rule` (default) is the z-score threshold the project ships with. `logreg` switches to a calibrated logistic-regression that maps `sentiment_index → P(up)`; confidence becomes `|2P − 1|` (a calibrated probability gap) rather than normalised z-distance. Fit / refit with `python -m finn_predictor.cli fit-classifier` after enough outcomes have closed (≥10 closed UP/DOWN predictions, both classes represented). The mode silently falls back to `rule` when no calibration is fitted yet, so flipping the env on a fresh deploy is safe. |
| `FINN_PREDICTOR_CLUSTERER` | `prefix` | How the *first reported* timestamp column is computed. `prefix` (default) is the dependency-free 8-word-prefix matcher. `embedding` switches to a sentence-transformers cosine-similarity clusterer (default model `all-MiniLM-L6-v2`, threshold 0.7); requires `pip install sentence-transformers` and downloads ~80 MB on first use. Failed embed calls log a warning and fall back to the prefix matcher automatically, so flipping the env never blanks the column. |
| (CLI) `refresh-constituents` | n/a | `python -m finn_predictor.cli refresh-constituents` is the headless mirror of the UI's *Focus → Sector → Refresh constituents* button. Reads `FINNHUB_API_KEY` from env. Required prerequisite for sector predictions and the cap-weighting fan-out. See `user-manual.md §6`. |
| `FINN_PREDICTOR_DISABLE_LOCAL_STORAGE` | unset | Set to `1` / `true` / `yes` to disable the browser-localStorage bridge that persists the Finnhub API key across page reloads. With the bridge active (default), the sidebar's API-key input survives `Ctrl+R` and the *Clear key* button wipes both server session and browser cache atomically. With the bridge off, the key is session-only — same security posture as pre-2026-05-20 builds. The package's frontend component polls forever in non-interactive runtimes (smoke tests, headless scripts), so the smoke-test fixture sets this automatically. |
| `FINN_PREDICTOR_MAGNITUDE` | `off` | Whether each prediction also carries a return-magnitude band. `off` (default) leaves the three `expected_return_*` columns null. `quantile` activates the pinball-loss quantile band (10th / 50th / 90th percentile) — but only takes effect once a calibration has been fitted via `python -m finn_predictor.cli fit-magnitude`. Requires ≥15 closed predictions with non-degenerate realised returns. Falls back silently to no-band when the calibration isn't present, so flipping the env on a fresh deploy is safe. |
